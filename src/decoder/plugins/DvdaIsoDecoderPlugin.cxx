...
bool        param_no_downmixes = true;
bool        param_no_short_tracks = true;
bool        param_no_untagged_tracks = true;
chmode_t    param_playable_area = CHMODE_BOTH;
std::string param_tags_path;
bool        param_tags_with_iso = false;
...
container_update(Path path_fs) {
    ...
    if (FileExists(curr_path)) {
        ...
        if (!param_tags_path.empty() || param_tags_with_iso) {
            ...
            dvda_metabase = std::make_unique<dvda_metabase_t>(
                reinterpret_cast<dvda_disc_t*>(dvda_reader.get()),
                param_tags_path.empty() ? nullptr : param_tags_path.c_str(),
                tags_file.empty() ? nullptr : tags_file.c_str());
        }
        dvda_path = curr_path;
    }
    ...
    for (auto track_index = 0u; track_index < dvda_reader->get_tracks(); track_index++) {
        if (dvda_reader->select_track(track_index)) {
            ...
            if (add_track) {
                ...
                char track_name[64];
                snprintf(track_name, sizeof(track_name), DVDA_TRACKXXX_FMT, track_index + 1, area, suffix);
